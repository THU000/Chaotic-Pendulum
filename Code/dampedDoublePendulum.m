...
g = 9.81;            % 重力加速度
Gamma = 1.2;         % 阻尼系数
Input = [L1,L2,m1,m2,M,g,Gamma]; % 双摆的物理模型参数,[L1,L2,m1,m2,M,g,Gamma]
y0=[pi/2;pi/2;0;0]; % 这是角度和角动量初始值
h=1e-2;
x0=0:h:30;
%代入到ODE求解器中
...
legend('$\dot{\theta_1}$','$\dot{\theta_2}$','Location','northeast',...
    'interpreter','latex', 'FontSize', 10);
xlabel('t(s)','interpreter','latex', 'FontSize', 18);
ylabel('theta_2_dot(rad/s)', 'FontSize', 18);
grid on;
...
function dydx = Fdydx(x,y,Input)
    ...
    D=(m2/3 + M)*L2^2;
    E=Gamma*(L1^3/3 + L1^2*L2 + 0.5*L1*L2^2*cos(th1-th2));
    F=Gamma*(L2^3/3 + 0.5*L1*L2^2*cos(th1-th2));
    G=Gamma*0.5*L1*L2^2*cos(th1-th2);
    H=Gamma*L2^3/3;
    ...
    dth1=pth1;
    dth2=pth2;
    dpth1=(B*(J-G*dth1-H*dth2)-D*(I-E*dth1-F*dth2))/(B*C-A*D);
    dpth2=(A*(J-G*dth1-H*dth2)-C*(I-E*dth1-F*dth2))/(A*D-B*C);
    %输出整理
    dydx=zeros(4,1);
    dydx(1)=dth1;
    dydx(2)=dth2;
